...
class FindUserByEmailTest extends TestCase
{
    ...
    {
        $request = new Request(['email' => 'fadeev4444@start2play.ru'], [], '');

        $usersRepository = $this->getUserRepository([
            new User(
                'Georgii',
                'Fadeev',
                'fadeev4444@start2play.ru'
            ),
        ]);
        ...
        $this->expectOutputString('{"success":true,"data":{"email":"fadeev4444@start2play.ru","name":"Georgii Fadeev"}}');
    ...
    private function getUserRepository(array $users): UserRepositoryInterface
    {
        ...
        return new class($users) implements UserRepositoryInterface
        {
            ...
            public function getUser($statementStub): User
            {
                throw new UserNotFoundException("Not found");
            }
